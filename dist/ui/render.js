...
export function renderUserBasicGrid(user) {
    let html = `
...
export function renderUserBasicList(user) {
    let html = `
...
export function createVisaRows(visas) {
    ...
    return visas
        ...
        const visaRow = createRow("visa-icon.svg", `Visa ${index + 1}`, `${v.issuing_country} - ${v.type}`);
        const dateRow = createRow("calendar-icon.svg", `Visa ${index + 1} ${expired}`, `${start} / ${end}`);
        return visaRow + dateRow;
    })
        .join("");
}
export function displayDetailedUser(user) {
    const userDetails = getElement(".user--details");
    let html = `
    <div class="avatar-section">
    <div class="wrapper">
      <img src="../${user.user_avatar}" alt="employee" class="avatar-section--img"/>
      ${user.isRemoteWork
        ? `<div class="home-box">
              <img src="../svgs/home-icon.svg" alt="home icon" class="avatar-section--home"/>
            </div>`
    ...
    ${createRow("calendar-icon.svg", "Date Of Birth", formatDateOfBirth(user.date_birth))}
    ${createRow("user-icon.svg", "Manager", `${user.manager.first_name} ${user.manager.last_name}`)}
    ...
    const copyUserLink = getElement(".avatar-section--copy");
...
export function renderHeaderLoggedIn(user) {
    let html = `
        <button
  ...
  `;
    const headerUser = getElement(".header--user");
